/*************************************************************************
    > File Name: lsrv_funcs1.c
    > Author: Duke-wei
    > Mail: 13540639584@163.com 
    > Created Time: 2017年10月18日 星期三 14时27分17秒
 ************************************************************************/
...
static char *do_validate(char *msg){
	int pid,slot;
	if(sscanf(msg+5,"%d.%d",&pid,&slot)==2&&ticket_array[slot]==pid) return "GOOD Valid ticket";
...
void handle_request(char *req,struct sockaddr_in *client,socklen_t addlen){
	...
	if(ret==-1) perror("SERVER sendto failed");
}
